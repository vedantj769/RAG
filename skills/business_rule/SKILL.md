...
kpi:oee, table:shift_oee_details_for_workcenter. `id`, `knowledge_name`,
...
Key properties that MAY additionally appear per label (use only if present in {schema}):
  BusinessRule  -> rule_name, condition, action, calculation, threshold, exception, priority, applicable_domain, applicable_hierarchy, effective_date
  KPI           -> kpi_name, unit, calculation_frequency
...
for that label, e.g.:
  WHERE toLower(r.id) CONTAINS toLower("long stop")
     OR toLower(r.knowledge_name) CONTAINS toLower("long stop")
